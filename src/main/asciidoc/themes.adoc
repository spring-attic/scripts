...
* Consul and etcd: provide an alternative implementation of the discovery abstraction in Spring Cloud Commons. Consul uses etcd as a datastore and cluster manager, so it has an emphasis on consistency (rather than availability, which is the focus of Eureka).
