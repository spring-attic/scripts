...
|spring.cloud.task.batch.fail-on-job-failurewait-time | 0 | Maximum wait time in milliseconds that Spring Cloud Task will wait for tasks to complete when spring.cloud.task.batch.failOnJobFailure is set to true.  Defaults to 0.  0 indicates no wait time is enforced.
