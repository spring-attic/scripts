...
|aws.paramstore.prefix | /config | Prefix indicating first level for every property.
...
|spring.cloud.stream.kafka.streams.binder.serde-error |  | {@link org.apache.kafka.streams.errors.DeserializationExceptionHandler} to use
...
|spring.cloud.stream.schema.server.path |  | Prefix for configuration resource paths (default is empty). Useful when embedding
